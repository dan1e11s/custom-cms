...
import { fetchHomePage, fetchSiteSettings } from '@/lib/api/site'
import { buildPageMetadata } from '@/lib/seo/metadata'
import { buildJsonLd } from '@/lib/seo/json-ld'
import type { Page } from '@/types/pages'

// ISR: ревалидируется по тегу 'home-page'
export const revalidate = 3600

export async function generateMetadata(): Promise<Metadata> {
  const page = await fetchHomePage()
  if (!page) return { title: 'Главная' }
  return buildPageMetadata(page as unknown as Page)
}
...
export default async function HomePage() {
  const [homePage, settings] = await Promise.all([fetchHomePage(), fetchSiteSettings()])
  ...
  return (
    <>
      ...
      <main>
        {homePage ? (
          /* ── Главная страница из CMS (конструктор блоков) ───────────────────── */
          <>
            {homePage.seo && (
              <script
                type="application/ld+json"
                dangerouslySetInnerHTML={{
                  __html: JSON.stringify(buildJsonLd(homePage as unknown as Page)),
                }}
              />
            )}
            <BlockRenderer
              blocks={homePage.blocks as Parameters<typeof BlockRenderer>[0]['blocks']}
            />
          </>
        ) : (
          /* ── Дефолтный контент (пока главная страница не настроена в CMS) ───── */
          <>
              ...
              <Container>
                <div className="mx-auto max-w-2xl text-center">
                  <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-1.5 text-sm text-muted-foreground">
                    ...
                    {settings?.siteName ?? 'CMS Platform'} — конструктор сайтов
                  </div>
